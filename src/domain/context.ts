...
const ACTION_PATTERNS: Record<string, string[]> = {
  ...
  persistence: ["/new", "/delete", "自動保存", "重設角色", "重置角色", "重設地圖", "重設圖鑑"],
...
export function prepareTurn(
  ...
  const runtimeContext = [
    ...
    "- 世界本體固定為艾爾維亞；不得在任何 State Diff 或 outcome_diff 傳入 world。世界發現與事件變化只能寫入地圖、人物、任務、圖鑑及歷史。",
    "- 遊戲內時間流逝使用 aegis_advance_time 並優先提供整數 elapsed_minutes；旅行或長事件若同時改變位置、地圖、人物、圖鑑或任務，使用 outcome_diff 與時鐘、生存及主要旅行事件在同一 revision 提交。不得直接改 player.clock/date/time/season，也不得在 outcome_diff.history 重複主要旅行事件。",
    "- 裝備或卸除物品只使用 aegis_equip_item / aegis_unequip_item，不得直接改寫裝備欄或複製物品。",
    "- 玩家要求清空或重設角色進度、地圖、人物知識或圖鑑時只使用 aegis_reset_player，不得逐欄位模擬清除；此操作永遠保留艾爾維亞世界本體。",
    "- 玩家位置只以 player.location.mapId 為權威；文字路徑由伺服器衍生。新地點、正式單向路線、新人物或新知識必須先與事件結果原子寫入，才能敘述為已發現；不得揭露 NPC 私密狀態、補造精確路線或世界全知資訊。",
    "- map/npcs/quests/compendium/history 使用穩定 ID。人物關係只用 label/tags；圖鑑內容使用具 factId、sources、confidence 的 facts；不得直接指定伺服器 revision／遊戲時間 metadata。",
