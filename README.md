...
- v0.6.0 舊存檔只遷移一次；遷移前建立最多保留三份的內部安全備份，公開狀態與私密狀態一起提交，失敗不提高 Schema 版本
...
地圖、人物與圖鑑是玩家知識投影，不是世界全知資料。新地點、新 NPC 或新知識必須與事件結果在同一筆成功交易中寫入後，才能出現在敘述與面板；沒有權威路線資料時，介面只會說明尚無資料，不會生成精確地圖。點擊節點、切換列表、拖曳、縮放及重新置中都是純前端操作，不會移動角色、推進時間或建立新回合。
...
驗收程式會使用隨機 `game_id` 測試 v0.6 旅行原子交易、時鐘、知識、冪等重試、私密 NPC 狀態、遷移備份、災難復原快照、重啟後持久性及並發面板鎖，再清除自己的測試資料。需要 SSL 時加上 `AEGIS_POSTGRES_E2E_SSL=true`。
