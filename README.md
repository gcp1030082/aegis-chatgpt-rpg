...
- 面板依 `gameId + revision` 拒絕重複或過期更新，頁籤切換不呼叫工具、不推進時間
...
| `aegis_prepare_turn` | 靜默唯讀 | 每個遊戲回合第一步，取得 Revision、相關狀態與規則，不渲染面板 |
...
| `aegis_show_dashboard` | 唯讀＋唯一 UI | 在回合所有寫入完成後，顯示角色、背包、裝備、技能、任務、地圖、人物、圖鑑與自動保存狀態 |
