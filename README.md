...
- 面板同時接收標準 MCP Apps 工具結果與 ChatGPT 延遲 globals 更新，並以新版模板 URI 避免手機沿用失敗快取
- 純讀取／重新整理面板時直接以 `aegis_show_dashboard` 作為單一工具呼叫，避開 ChatGPT 手機版在同回合多工具後漏掛 UI 的問題；伺服器原子建立展示回合且不改時間、State 或 revision
- 唯一固定世界「艾爾維亞」：劍與魔法、多種族、魔物、地下城、公會、王國、宗教、鍊金與魔導技術並存；玩家與模型工具都不能改寫 `world` 本體
- 可拖曳、縮放、重新置中的 SVG 節點地圖與地點列表；空間階層、單向旅行路線、地點危險與路線危險分開呈現
- `mapId` 是玩家位置唯一權威；名稱路徑只由伺服器依 region／town／place／subplace 階層衍生
- `mapId`、`routeId`、`facilityId`、`dangerId`、`npcId`、`infoId`、`serviceId`、`memoryId`、`questId`、`entryId`、`factId`、`eventId` 穩定識別與完整候選狀態交叉引用驗證
- 玩家已知 NPC 與伺服器私密 NPC 世界狀態分離儲存；秘密、真實位置及對話逐字稿不進入 GameView、Dashboard 或前端
- 圖鑑依傳聞、觀察、辨識、驗證、研究逐步提升；每項 fact 具有來源、可信程度、去重與伺服器學習時間 metadata
- 完整世界曆法與權威 GameClock，可由伺服器處理跨日、跨月、跨季節與跨年；日期、時間、季節只是衍生快取
- `aegis_advance_time` 優先接受整數 `elapsed_minutes`，相容舊 `elapsed_hours`，並把旅行時間、生存、位置、路線、NPC、任務、圖鑑與歷史原子提交在同一 revision
- 結構化繁中歷史格式化器；未知舊事件使用安全文字，不把原始 JSON 顯示給玩家
...
- 新版客戶端會把 `turnId` 傳回面板工具；若 ChatGPT 仍快取只有 `game_id` 的舊 schema，伺服器會原子認領目前有效回合，保留單回合一次面板鎖
- 面板依 `gameId + turnId + revision` 拒絕同回合重複或較舊更新，並允許新回合在未產生 revision 時正常顯示；頁籤切換不呼叫工具、不推進時間
- Dashboard DTO 只傳局部地圖及有界的人物、圖鑑、歷史詳情；100 個權威節點時前端最多建立 40 個可見節點
...
- 原子化玩家進度重設：同一交易清除角色、物品、裝備、任務、歷史、地圖、人物知識、圖鑑及私密人物進度，固定世界艾爾維亞與開發者災難復原快照保持不變
...
- v0.7.0 舊存檔只遷移一次並把世界本體固定為艾爾維亞；遷移前建立最多保留三份的內部安全備份，公開狀態與私密狀態一起提交，失敗不提高 Schema 版本
...
玩家訊息
  ↓
ChatGPT ── 一般回合：靜默 aegis_prepare_turn ──→ MCP Server ──→ 權威 State
  │                                      │
  ├─ 判定與繁中敘事                      ├─ Validator
...
純面板請求：ChatGPT ── 唯一一次 show_dashboard ──→ 不改資料的展示回合
```
...
| 工具 | 類型 | 用途 |
|---|---|---|
| `aegis_create_game` | 寫入 | 在固定世界艾爾維亞建立新遊戲，不覆蓋既有 `game_id` |
| `aegis_get_game_state` | 靜默唯讀 | 查詢完整權威狀態，不渲染面板、不推進時間 |
| `aegis_prepare_turn` | 靜默控制 | 一般遊戲行動的第一步，取得 Revision、相關狀態與規則，並簽發不改變遊戲 State 的 `turnId`；純面板請求不呼叫 |
| `aegis_apply_state_diff` | 寫入 | 驗證並原子提交一回合的 State Diff；禁止傳入 `world` |
| `aegis_reset_player` | 破壞性寫入 | 原子重設玩家進度、地圖、人物知識、圖鑑與私密人物進度；保留艾爾維亞及內部快照 |
| `aegis_advance_time` | 寫入 | 以 `elapsed_minutes`（相容 `elapsed_hours`）推進權威 GameClock、結算生存消耗，並原子提交完整旅行結果 |
...
| `aegis_show_dashboard` | 唯一 UI＋原子鎖 | 純面板請求時作為第一個且唯一工具並由伺服器建立展示回合；一般回合則帶入 `turnId`，在所有寫入完成後顯示最新狀態 |
...
艾爾維亞的 `world` 是伺服器定義且不可由玩家工具修改的世界本體。地圖、人物、任務、圖鑑與歷史則是玩家在艾爾維亞中的進度與知識投影，可以隨探索增加，也能隨玩家進度重設清空。新地點、新 NPC 或新知識必須與事件結果在同一筆成功交易中寫入後，才能出現在敘述與面板；沒有權威路線資料時，介面只會說明尚無資料，不會生成精確地圖。點擊節點、切換列表、拖曳、縮放及重新置中都是純前端操作，不會移動角色、推進時間或建立新回合。
...
驗收程式會使用隨機 `game_id` 測試 v0.7 固定艾爾維亞、旅行原子交易、時鐘、知識、玩家與私密人物進度重設、冪等重試、遷移備份、災難復原快照、重啟後持久性及並發面板鎖，再清除自己的測試資料。需要 SSL 時加上 `AEGIS_POSTGRES_E2E_SSL=true`。
...
正式部署建議使用 PostgreSQL。Server 會自動建立公開權威狀態、私密世界狀態、回合鎖、內部遷移備份與災難復原快照所需資料表；State 以 JSONB 保存，私密 NPC 資料不會混入玩家 GameState。
